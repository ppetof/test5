...
The hero is a dark stage: type on the left, an interactive Spline scene on the
right, and a soft light that follows the cursor across both. The photograph
that used to live here is gone.

- **Scene** — `https://prod.spline.design/kZDDjO5HuC9GJUM2/scene.splinecode`,
  ~1.3 MB, loaded at runtime from Spline's CDN.
- **Runtime** — vendored in `vendor/`, not pulled from a CDN. `@splinetool/runtime`
  splits into lazily imported chunks (`process.js`, `physics.js`, `ui.js` and
  others); the entry file alone fails at load time with a bare
  "Failed to fetch dynamically imported module". **`vendor/` must ship with the
  site** — it is not a build artefact and must never be gitignored.
- **`media/robot-poster.jpg`** — a still captured from the scene itself. It is
  the hero until the scene is live, and it stays the hero permanently under
  reduced motion, Save-Data, or if the scene fails to load. There is no state
  where the hero is empty.

Rules:

- **Nothing here is part of first paint.** The scene boots from an
  IntersectionObserver, and the still holds the frame until `.is-3d` lands.
  Total transfer measured ~3.5 MB — acceptable for a hero the visitor chose,
  unacceptable as a blocking load.
- **`spline.setGlobalEvents(true)` is required, not optional.** By default the
  scene only tracks a pointer that is over its own canvas, so the robot sits
  frozen while the visitor's cursor is on the headline — which is where it
  spends most of its time. Global events let it follow the pointer across the
  whole page. Verified not to hijack wheel scrolling or nav clicks.
- **Verify tracking by looking at the head, not at a pixel diff.** The cursor
  light and the scene's idle animation both change pixels every frame, so
  "pixels differ" proves nothing here. Park the cursor at two positions, let
  it settle, and compare where the head is pointing.
- **The light is independent of the scene.** `.hero__spot` is CSS driven by
  pointer coordinates, so it still works when WebGL is unavailable.
- On phones the robot drops to `opacity: .55` and sits behind the type rather
  than beside it; the type keeps the left 56% only above 900px.
- The robot is a generic humanoid, not enduro subject matter. It is here
  because the client asked for it by name, not because it argues the product.
