...
## Hero: the scroll-scrubbed climb

The hero is a 4-second clip of a rider forcing the bike up a rock face
(`Hero_video.mp4`, taken from `../test_2`), scrubbed by scroll position. The
visitor scrolls, the rider climbs — the interaction and the subject say the
same thing.

- `.hero` is `260svh` (`230svh` under 900px). **That height is the timeline**;
  shortening it shortens the scrub.
- `.hero__sticky` pins the frame for the whole of it.
- Scroll sets `video.currentTime`. Playback is never started.

Rules that must survive any future change:

1. **The clip is encoded all-keyframe** (`--gop 1`, `media/hero-scrub-*.mp4`).
   Scrubbing seeks to arbitrary times, and with a normal GOP the decoder walks
   from the previous keyframe on every seek, which stutters. All-keyframe costs
   bitrate and buys exact, instant seeks: 2.9 MB desktop, 1.6 MB mobile for 4s.
   A frame sequence was measured first and rejected — this foliage cost 120 KB
   a frame, 5.6 MB for only 48 steps.
2. **`body` uses `overflow-x: clip`, never `hidden`.** `hidden` turns the body
   into a scroll container and silently kills `position: sticky` — the hero
   frame scrolls away and leaves a black band. This bit once already.
3. **Seeks are coalesced through rAF** and ignored below a 12ms delta, so a
   fast scroll does not queue a decode per scroll event.
4. `reduce` and `saveData` never load the clip; the poster still is the hero.

### Type drift

Every hero text element carries `data-drift="N"`, its own rate, and the pointer
moves it: slogan 3, headline 5, date 8, stat 12, video badge 14. The graduation
is the effect — uniform movement reads as a sliding sheet, not depth.

**A transform-based entrance animation cannot share an element with a drifter.**
`transform: none` in the entrance rule overwrites the drift transform and the
element sits frozen while its neighbours move. The slogan and the video badge
enter on opacity alone for exactly this reason.
