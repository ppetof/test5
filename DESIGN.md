...
- **Both plates share `object-fit: cover` and `object-position: 58% 50%`.**
  ...
  composite apart.
- **The foreground is scaled `1.21` (1.19 on mobile).** This is not only for
  emphasis — it must stay large enough to cover the repaired patch at full
  parallax travel. The repair is dilated 55px and feathered 20px at source
  scale; if either the scale drops or the dilation grows, the patch shows as a
  pale haze beside the rear wheel. Check the extremes, not the rest position.
