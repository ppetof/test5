...
**The hero photograph is no longer used.** At the client's request the hero is
an interactive Spline 3D robot scene (`vendor/` runtime plus a remote scene
file). `main image.jpeg` remains in the folder as their source asset but is not
referenced by the site. The robot is generic stock subject matter and says
nothing about the race — it is a client-chosen decoration, not evidence.
